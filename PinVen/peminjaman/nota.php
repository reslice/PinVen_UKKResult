...
<body>
    ...
    <form align="center"  action="process/add-process.php" method="POST">
        ...
        Tanggal Kembali <input type="date" value="<?= date('Y-m-d'); strtotime("date+1")?>" name="tanggalKembali">
        Jumlah <input type="number" name="jumlah">
        <select name="idPegawai" id="">
            <?php foreach($pegawai->all() as $detailPegawai ){?>
                <option value="<?= $detailPegawai['id_pegawai']?>"> <?= $detailPegawai['nama_pegawai'] ?> </option>
            <?php } ?>
        </select>
